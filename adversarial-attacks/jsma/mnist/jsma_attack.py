...
with tf.Graph().as_default():
  with tf.Session() as sess:
    ...
    x_adv_1 = jsma.generate_np(adv_inputs[:10], **jsma_params)
    x_adv_2 = jsma.generate_np(adv_inputs[10:20], **jsma_params)
    x_adv_3 = jsma.generate_np(adv_inputs[20:30], **jsma_params)
    x_adv_4 = jsma.generate_np(adv_inputs[30:40], **jsma_params)
    x_adv_5 = jsma.generate_np(adv_inputs[40:50], **jsma_params)
    
    x_adv = np.empty((0,28,28,1))
    for i in range(0,adv_inputs.shape[0]):
        x_adv_tmp = jsma.generate_np(adv_inputs[i:i+1], **jsma_params)
        x_adv = np.concatenate((x_adv, x_adv_tmp), axis=0)
