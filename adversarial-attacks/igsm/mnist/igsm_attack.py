...
with tf.Graph().as_default():
  with tf.Session() as sess:
    ...
    igsm_params = {'eps': 0.3,
                 'clip_min': 0.,
                 'clip_max': 1.}
    x_adv_1 = igsm.generate_np(adv_inputs[:10], **igsm_params)
    x_adv_2 = igsm.generate_np(adv_inputs[10:20], **igsm_params)
    x_adv_3 = igsm.generate_np(adv_inputs[20:30], **igsm_params)
    x_adv_4 = igsm.generate_np(adv_inputs[30:40], **igsm_params)
    x_adv_5 = igsm.generate_np(adv_inputs[40:], **igsm_params)
    ...
    for i in index_list:
      if np.argmax(model.predict(x_adv[i:i+1])) != np.argmax(adv_labels[i]) and classes[np.argmax(adv_labels[i])] < 105:
        tifffile.imsave("examples/%d.tif" % j, x_adv[i])
